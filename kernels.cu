...
namespace kernels {
    ...
    __global__ void rank1_update_kernel(float* d_cov_matrix, const float* d_new_row, float* d_mean, const int cols, const int n) {
        ...
        for (int j = 0; j < cols; ++j) {
            ...
            d_cov_matrix[row_start + j] = fmaf(
                delta_old,
                delta_new / (n - 1),
                (n - 2) / static_cast<float>(n - 1) * d_cov_matrix[row_start + j]
            );
        }
    ...
    // updates a single chunk of a covariance matrix row, so that concurrently launched versions of this kernel can
    // collaboratively calculate a covariance row
    __global__ void update_covariance_chunk(float* d_cov_matrix, const float* d_new_row, float* d_mean, const int cols, const int n, const int chunk_start, const int chunk_size) {
        ...
        if (const int col_idx = chunk_start + idx; col_idx < chunk_start + chunk_size && col_idx < cols) {
            const float new_mean = fmaf(d_new_row[col_idx] - d_mean[col_idx], 1.0f / n, d_mean[col_idx]);
            const int row_start = col_idx * cols;
